...
SELECT EMP_NAME, (SALARY * 12) AS "연봉",
      ...
      (((SALARY + (SALARY * NVL(BONUS, 0))) * 12) - (SALARY *12) * 0.3)  AS "실수령액"
FROM EMPLOYEE;
...
SELECT  EMP_NAME,
        EXTRACT(YEAR FROM HIRE_DATE) AS "생년", EXTRACT(MONTH FROM HIRE_DATE) AS "생월", EXTRACT(YEAR FROM HIRE_DATE) AS "생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, ABS(TRUNC(HIRE_DATE - SYSDATE)) AS "근무일수1", ABS(TRUNC(SYSDATE - HIRE_DATE)) AS "근무일수2"
FROM EMPLOYEE;
...
SELECT *
FROM EMPLOYEE
--WHERE SUBSTR(EMP_ID, -1) IN ('1', '3', '5', '7', '9');
--WHERE MOD(EMP_ID, 2) = 1; --자동형변환이 일어남_
...
SELECT *
FROM EMPLOYEE
--WHERE  EXTRACT(YEAR FROM HIRE_DATE) > 20 OR  EXTRACT(YEAR FROM HIRE_DATE) < 04;
--WHERE ADD_MONTHS(HIRE_DATE, 240) < SYSDATE;
WHERE MONTHS_BETWEEN(SYSDATE, HIRE_DATE) > 240;

-- 20. EMPLOYEE 테이블에서 사원명, 급여 조회 (단, 급여는 '9,000,000' 형식으로 표시) 

SELECT EMP_NAME, TO_CHAR(SALARY, '9,000,000') AS "급여"
FROM EMPLOYEE;
...
--       TRUNC(CEIL(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1,6)))) /12) AS "나이(만)"
...
SELECT EMP_NAME, SUBSTR(EMP_NO, 1,6) AS "주민번호 앞자리", 
                 SUBSTR(EMP_NO, 8,7) AS "주민번호 뒷자리", 
                 SUBSTR(EMP_NO, 1,6) + SUBSTR(EMP_NO, 8) AS "주민번호 앞자리와 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
/*
SELECT TO_CHAR(TO_DATE(HIRE_DATE, 'YY/MM/DD'), 'YY') AS HIRE_YEAR, COUNT(*) AS EMP_COUNT
FROM EMPLOYEE
GROUP BY TO_CHAR(TO_DATE(HIRE_DATE, 'YY/MM/DD'), 'YY');
UNION
SELECT 'TOTAL', COUNT(*)
FROM EMPLOYEE;
*/