...
/*
     ...
     *트랜잭션
     -데이터베이스의 논리적 연산단위
     -데이터의 변경사항(TML)등을 하나의 트랜잭션에 묶어서 처리한다.
     DML문 한개를 수행할 때 트랜잭션이 존재하지 않는다면 트랜잭션을 만들어서 묶음.
                                   존재한다면 헤당 트랜잭션에 묶어서 처리한다.
     ...
     SAVEPOINT 포인트명 : 현 시점에 해당 포인트 명으로 임시저장을 해두겠다.
...
SELECT * FROM EMP_01;
--SAVEPOINT SP로 돌아가기 즉, 김말똥, 김말순 추가한거는 삭제
ROLLBACK TO SP;
--트랜잭션 저장
COMMIT;
...
/*
    ...
    ACID속성 : 트랜잭션에 일관성과 신뢰성을 보장하기 위해 지켜야하는 네가지 핵심 속성
    원자성 : 트랜잭션 내의 작업이 모두 완료(COMMIT)되거나, 모두 취소(ROLLBACK)되어야 함.
    일관성 : 트랜잭션이 시작되기 전과 후에 데이터베이스의 상태가 항상 일관성을 유지해야함.
    독립성 : 동시에 실행되는 트랜잭션이 서로에게 영향을 끼치지 않도록 독립적으로 처리되어야 한다.
