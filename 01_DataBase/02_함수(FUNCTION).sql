SELECT EMP_ID, EMP_NAME, SALARY    --3순위
FROM EMPLOYEE                      --1순위
WHERE DEPT_CODE = 'D2';            --2순위

/*
     <ORDER BY절>
     SELECT 문 가장 마지막 줄에 작성, 성행 순서 또한 가장 마지막에 실행한다.
     ...
     FROM 조회할 테이블
     WHERE 조건식
     ORDER BY 정렬기준이 될 컬럼명 | 별칭 | 컬럼 순번 [ASC | DESC] [NULLS FIRST | NULLS LAST]
     []는 생략 가능한 명령어들
     
     ASC : 오름차순 (작은 값으로 시작해서 점점 커지는 것) -> 기본값
     DESC : 내림차순 (큰 값으로 시작해서 점점 커지는 것) 
     
     --NULL은 기본적으로 가장 큰 ㅏㅄ으로 분류해서 정렬한다.
     NULLS LAST :  정렬하고자하는 컬럽값에 NULL이 있을 경우 해당 데이터 맨 뒤에 배치(ASC일 때 기본값)
...
--ORDER BY BONUS DESC; --[NULLS FIRST]가 기본값으로 적용
--정렬기준의 컬럼 값이 동일할 경우 그 다음차순을 위해서 여러개를 제시할 수 있다.
...
/*
     ...
     -그룹 함수 : N개의 값을 일거들여서 1개의 결과값을 리턴 (그룸을 지어 그룹별로 함수 실행 결과를 반환)
     
     >>SELECT 절에 단일행 함수와 그룹함수를 함께 사용하지 못한다.
     왜?      결과행의 갯수가 서로 다르기 때문에
     
     >>함수식을 사용할 수 있는 위치 : SELECT절 SHERE절 ORDER BY절 GROUP BY절 HAVING절
...
SELECT LENGTH('ORACLE'), LENGTHB('OACLE')
FROM DUAL;
...
-- 찾을 위치의 시작값 : 1, 찾고자 하는 문자의 순번 : 1
SELECT INSTR('AAABAACAABBAA', 'B', -1) FROM DUAL;
-- 찾을 위치의 시작 값 : -1, 뒤에서 부터 찾지만 읽을 때는 원래대로 읽어준다.
SELECT INSTR('AAABAACAABBAA', 'B', 1, 2) FROM DUAL;
-- 찾을 위치의 시작값 : 1, 찾고자 하는 문자의 순번 : 2 (문자열 중에서 두번째로 나오는 찾고자 하는 문자의 위치)
...
--20만큼의 길이 중 EMAIL컬럼 값은 오른쪽으로 정렬하고 나머지 부분은 공백으로 채운다.
...
--사원들의 사원명, 주민등록번호("123456-XXXXXXX")
--SELECT EMP_NAME, RPAD(SUBSTR(EMP_NO, 1, 8), 14, 'X')
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 8) || 'XXXXX'
FROM EMPLOYEE;
...
/*
    ...
    *LTRIM/RTRIM
    ...
    LRTIM/RTRIM(컬럼 | '문자열', [제거하고자하는 문자들])
    
    문자열의 왼쪽 혹은 오른쪽에서 제거하고자하는 문자들을 찾아서 나머지 문자열 반환
...
SELECT TRIM(BOTH 'Z' FROM 'ZZZZZZZZZZZKHZZZZZZZ') FROM DUAL; --양쪽
SELECT TRIM(LEADING 'Z' FROM 'ZZZZZZZZZZZKHZZZZZZZ') FROM DUAL; --LTRIM 유사
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZZZZZZZZKHZZZZZZZ') FROM DUAL; --RTRIM 유사
...
/*
     *LOWER / UPPER / INITCAP
     ...
     INITCAP : 띄어쓰기 기준 첫 글자마다 대문자로 변경한 문자열을 반환
...
SELECT TRUNC(123.987) FROM DUAL;  --기본자수는 소수점 첫번째 자리에서 내림
SELECT TRUNC(123.987, 1) FROM DUAL;  --양수로 증가할 수록 소수점 뒤로 한칸씩 이동
SELECT TRUNC(123.987, -1) FROM DUAL;  --음수로 감소할 수록 소수점 앞자리로 이동
