...
/*
    ...
    (MAXVLAUE 숫자) -> 최대값지정(기본값 매우 크다)
    (MINVALUE 숫자) -> 최소값지정(기본값 1)
    (CYCLE | NOCYCLE) -> 값 순환여부(기본값 NOCYCLE)
    (CACHE | NOCACHE) -> 캐시메모리 항당(기본값 CACHE 20)
    
    *캐시메모리 : 미리 방생될 값들을 생성해서 저장해두는 공간
...
--NEXTVAL를 한번도 수행하지 않는 이상 CURRVAL를 할 수 없다.
...
SELECT SEQ_EMPNO NEXTVAL FROM DUAL; --315 오류방생 -> 최대값이 310
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
            VALUES(SEQ_EID.NEXTVAL, '김말순', '111111-2111111', 'J6', SYSDATE);
