/*
    ...
    SQL문장 내에서 변수의 정의, 조건(IF), 반복(FRO, WHILE)등을 지원하여 SQL단점을 보완
    다수의 SQL문을 한번에 실행 가능
    
    PL/SQL구조(블록구조)
    - [선언부] : DECLARE로 시작, 변수나 상수를 선언 및 초기화 하는 부분
    - [실행부] : BEGIN으로 시작, SQL문 또는 제어문등의 로직을 기술하는 부분 
    - [예외처리부] : EXCEPTION으로 시작, 예외 발생시 해결하기 위한 구문
...
-- 래퍼런스 타입변수 선언 및 초기화(어떤 테이블의 어떤 컬럼의 데이터타입을 참조해서 그 타입으로 지정
DECLARE
...
/*
    ...
    상자가 입력한 사번의 사번, 사원명, 직급코드, 급여, 부서명 조회 후 각 변수에 담아서 출력
...
--1_3)ROW타입 변수 선언
--테이블의 한 행에 대한 모든 컬럼값을 한변에 담을 수 있는 변수
...
--3) IF 조건식 THEN 실행내용 ELSIF 조건식2 THEN 실행내용2....(ELSE 실행내용) END IF;
