/*
    ...
    <트리거>
    내가 지정한 테이블에 INSERT, UPDATE, DELETE등 DML문에 대한 변경 사항이 생길 때
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있다.
    
    EX)
    회원탈퇴 시 직원의 회원테이블에 데이터 DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자도으로 INSERT 시켜야한다.
    신고 횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔
    입출고에 대한 데이터 기록(INSERT)될때마다 해당 상품에 대한 재고수략을 매번 수저(UPDATE)해야한다.
    
    *트리거의 종류
     - SQL문의 실행시기에 따른 분류
       - BEFORE TRIGER : 내가 지정한 테이블에 이벤트가 발생되기 전에 트리거 실행
       - AFTER TRIGER : 내가 지저한 테이블에 이벤트가 발생된 후에 트리거 실행
...
CREATE OR REPLACE TRIGGER TG_01
...
-- TB_PRODETAIL 테이블에 INSET 이벤트 발생시
...
/*
    ...
    UPDATE TB_PRODUCT
    SET STOCK = STOCK + 입고된수량(INSERT된 자료의 AMOUT)
    WHERE PCODE = 입고된상품번호(INSERT된 자료의 PCODE);
    
    --상품이 출고 -> 해당상품찾아서 재고수량 감소 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK - 입고된수량(INSERT된 자료의 AMOUT)
    WHERE PCODE = 입고된상품번호(INSERT된 자료의 DECODE);
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL, 200, SYSDATE, 7, '입고');
