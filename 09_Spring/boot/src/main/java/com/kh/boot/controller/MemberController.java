...
public class MemberController {
    ...
    /*
      ...
      클래스내에서 필요한 개체를 직접 생성하지않고 spring컨테이너가 관리하는 객체(Bean에 등록)을 주입받아서 사용할 수 있게 해줌
      필드주입방식/생성자주입방식

      private MemberService memberService = new MemberServiceImpl();
      기존 객체 생성방식
      객체간의 결합도가 높아짐(소스코드 수정이 일어나면 하나하나 전부 바꿔줘야한다.)
      서비스가 동시에 매우 많은 요청이 될 경우 그만큼 많은 객체가 생성된다.
      ...
      단점 : 테스트가 어렵다(필드주입방식은 객체생성시 의존성이 주입되지 않고 bean에서 생성 후 주입받는 방식이기 때문에
                          테스트 진행시 임의의 객체를 생성하기가 어렵다.)
            불변성을 보장할 수 없음. 객체 생성시 의존성이 주입되어 고정되지 않기 때문에 클래스 생성 이후에도 의존성이 변경될 수 있음

     생성자 주입방식
     스프링 컨테이너가 객체를 생성할 때 @Autowired어노테이션이 붙은 생성자를 통해 필요한 의존성을 주입하는 방식
     장점 : 불변성 보장, 테스트가 편리하다, 순환참조방지

      private final MemberService memberService;만 작성하고 @RequiredArgsConstructor으로도 생성가능
     */
    ...
    /*
        ...
        메소드에 배개변수로 HttpServletRequest를 작성해주면
    ...
    */
    /*
        ...
        request.getParameter(키)로 밸류를 추출하는 역할을 대신 해주는 어노테이션이다.
        요청 parameter의 key값과 동일하게 매개변수명을 설정해주면 @RequestParam어노테이션을 생략해도 됨
...
*/
    ...
    public ModelAndView login(@ModelAttribute Member member, ModelAndView mv, HttpSession session){
        ...
        //bCryptPasswordEncoder.matches(평문, 암호문) --> 해당 비밀번호가 암호하된 비밀번호와 일치하면 true 아니면 fasle 반환
...
//        session.invalidate(); //세션 만료
    ...
    public String insert(Member member, HttpSession session, Model model){
        /*
            age 값은 int로 필드를 구성할 경우 빈 문자열이 전달되면 형변화 과정에서 400에러가 발생한다.
            보통 400에러는 보내는 데이터와 받는 데이터의 타입이 일치하지 않을경우 발생한다.
            
            비밀번호를 사용자 입력 그대로 저장한다.(평문)
            Bcrypt방식을 이용해서 암호화 작업 후  저장함
       ...
       if(result > 0) {
           session.setAttribute("alertMsg", "성공적으로 회원등록을 완료하였습니다.");
           return "redirect:/";
       } else {
    ...
    public String updateMember(Member member, HttpSession session, Model model){
        ...
        if(result > 0) {
            session.setAttribute("alertMsg", "회원정보 수정이 완료하였습니다.");
            return "member/myPage";
        } else {
            model.addAttribute("errorMsg", "회원가입 실패");
            return "common/errorPage";
