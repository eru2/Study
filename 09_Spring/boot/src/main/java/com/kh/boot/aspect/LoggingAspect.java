...
/*
    스프링 어플리케이션은 대부분의 경우에서 MVC패턴에 따라서 Web Layer -> Business Layer -> data Layer로 동작
    ...
    AOP를 통해서 이러한 각 레이어에서 반복되는 공통 관심사항을 분리함으로써 코드의 재사용성을 늘리고
    유지보수성을 강화할 수 있다.
    ex) 로깅, 트랜잭션관리, 보안...
    ...
    단점
    - AOP설정을 추가해줘야하기 때문에 조금 복잡하다.
 */
/*
        <시점>
        @Before : 대상메서드 실행 전에 Advice(추가 기능)가 실행된다.

        @After : 대상메서드 실행 후 Advice(추가기능)이 실행된다.

        @AfterReturning : 대상 메서드가 정상적으로 반환된 후에 Advice(추가기능)

        @AfterThrowing : 대상메서드가 비정상적으로 반환(예외를 반환)된 후에 Advice(추가기능)
...
public class LoggingAspect {
    ...
    public void afterThrowing(JoinPoint joinPoint, Exception e){
        log.info("============================== ERROR =============================="); //log.info는 일반적인 정보를 남길 때
        log.info("error in request : {}", joinPoint.getSignature()); //joinPoint.getSignature() = methodSignature
        log.info("error msg : {}", e.getMessage());
    }
    
    @Around("controllerPointcut()")
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable{ //ProceedingJoinPoint 실행될 개체를 같이 넘겨줌
