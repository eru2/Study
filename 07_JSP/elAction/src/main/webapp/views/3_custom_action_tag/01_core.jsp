...
<!-- prefix="c"를 통해서 core의 접두어를 c로 선언 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<body>
    ...
    *변수 선언과 동시에 초기화(c:set var="변수명" value="값" [scope="저장객체"])
    ...
    <c:set var="num1" value="10" /> <%-- <% pageContext.setAttribute("num1", 10); %> 앞에 것과 같다--%>
    ...
    *변수 삭제(c:remove var="제거하고싶은 변수명" [scope=""])
    -해당 scope영역에 해당 변수를 찾아서 제가하는 태그
    -scope 지정 생략시 모든 scope에서 해당 변수를 다 찾아서 제거한다.
    => 즉, 해당 scope에 .removeAttibute()를 통해 제거하는 것과 동일
    </pre>
...
<br><br>
        ...
        <c:set var="color" value="red,yellow,green,pink" />
        colors : ${colors} <br>
    ...
    %>
        ...
        <a href="list.do?color=balck&item=5">기존방식 요청</a> <br>

        <c:url var="llistUrl" value="list.do">
