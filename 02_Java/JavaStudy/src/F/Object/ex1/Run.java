...
public class Run {
	 ...
	 *           
	 * 3. 객체지향 프로그래밍 : 현실세계의 객체들 간의 상호작용 -> 프로그래밍을 통해서 가상세계로 구현하는 방법
	 * 
	 * 4. 구현하고자하는 프로그램상의 필요한 객체들을 만들기 위해서(생성하기 위해서)
	 *    :클래스라는 설계도가 먼저 필요하다.
	 *    
	 * 5. 추상화 과정
	 *    1) 내가 구현하고자 하는 프로그램에서 필요한 객체는 뭘까? 생가해보아야 한다.
	 ...
	 *    
	 *    ex) 학생 관리 프로그램
	 *     1) 학생 관련 객체(김민수훈련생... 최지원훈련생... 등등)
	 *     2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기 속도)
	 *     3) 학생 "인적사항"을 관리하는 프로그램 : 이름, 나이, 주소, 전화번호...
	 *        학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수, 성장률...
	 *        학생 "건강"을 관리하는 프로그램 : 이름, 반, 달리기 속도, 점프력, 키, 몸무게, 혈압.....
	 *     4) 이름 : String name;
	 *        나이 : int age;
	 *        키 : double height;
	 *        
	 ...
	 *         나이를 보관하는 배열 int[] age = {55, 60... } -> name과 age간의 순서를 보지 않는다.
	 *     
	 *     3) 그래서 나온 개념 -> 구조체
	 *     구조체 : 여러 개의 자료형의 여러개의 값들을 보관할 수 있다. -> 나만의 타입을 만들 수 있음
	 *     Struct human{
	 *         String name;
	 *         int age;
	 *         }
