...
//		(Sonata)c1;  ? -> 실제로 만들어진 공간이 Sonata가 아니다. (업캐스팅되지 않았다.) 다운캐스팅 불가
		...
		((Car)c2).drive(); // 부모참조변수로 업캐스팅 가능 -> 오버라이딩 된 메서드는 생성된 메모리를 기준으로 호출됨
//		((Car)c2).moveAvante() // 업캐스팅시 자식요소의 메모리 접근 불가
		
		// 3. 부모타입 래퍼런스로 자식객체를 다루는 경우(업캐스팅)
		Car c3 = new Sonata("검은색", "디젤", 2015);
		c3.drive();
//		c3.moveSonata(); 부모타입에 참조변수이기 때문에 자식요소의 메서드 접근 불가
//		(Sonata)c3.moveSonata(); c3.의 (.)과 (Sonata).의 (.)이 우선순위 충돌 
