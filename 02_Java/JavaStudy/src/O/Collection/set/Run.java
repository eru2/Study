...
public class Run {
	...
	public static void main(String[] args) {
		...
		//String에 hashCode() 오버라이딩 -> "실제 담긴 문자열"을 가지고 10진수 형태로 해시코드로 반환
		...
		//동일객체 : 개체마다 hashCode와 equlas의 결과가 모두 일치(true)하는 객체
...
//		hs2.get() -> 인덱스의 개념이 없기 때문에 get을 할 수 없음
		...
		//hashSet에 담긴 모든 개체를 순차적으로 접근
		// 1. for each문 이용
		for(Student st : hs2) {
			System.out.println(st);
		}
		
		// 2. Iterator 인터페이스를 이요해서 순차적 접근
		// Iterator : 컬렉션 요소에 순차저으로 접근하기위한 반복자 인터페이스
		// hasNeext() : 다음요소가 있는지를 알려줌(true/false)
		...
		// 3. ArrayList에 담아준 다음 ArrayList를 반복적으로 돌아가며 접근
