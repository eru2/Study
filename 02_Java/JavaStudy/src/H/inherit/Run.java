...
public class Run {
	 ...
	 *  - 코드를 공통으로 관리하기 때문에 코드의 추가나 변경이 용이하다(유지봇성이 좋음)
	 *  
	 * 상속의 특징
	 *  - 클래스간의 다중상속은 불가(부모는 하나다)
	 *  - 부모클래스에 정의되어있는 protected 필드는 자식클래스에서 직접 접근이 가능하다(private는 불가)
	 *  - 자식객체는 부모클래스에 있는 메서드, 필드를 마치 내것처럼 호출할 수 있음(this로 호출)
	 *  + 부모클래스에 정의된 기능이 마음에 들지 않는다면 내 마음대로 수정할 수 있음(오버라이딩)
