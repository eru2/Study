...
public class B_String {
	...
	public void method01() {
		...
		// String 클래스에서 equals메서드는 이미 오버라이딩 되어있음(주소값비교가 아닌 값 비교)
		
		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode());
		//String 클래스에서 hashCode메서드도 이미 오버라이딩 되어있음(주소값이 아닌 문자열을 가지고 만듬)
		...
		str3 = "bye";
		System.out.println(System.identityHashCode(str3));
		//변경하는 순간 기존의 문자열 자리에서 변경되는게 아니라 새로운 값을 참조하도록 되었다. (숭수플에 새로운 값이 등록되고 해당 주소를 가르킴)
	}
	
	public void method02() {
		String str1 = "Hello Wrold";
		
		//문자열.charAt(ind index) : char
		//문자열에서 전달바든 index위치에 문자를 추출해서 반환
		...
		System.out.println("str1에 bye는 문자열 포함? : " + str1.contains("bye"));
		
		//문자열.substring(시작위치, 끝위치)
		//문자열을 시작위치부터 끝위치 -1까지 추출해서 반환한다
		System.out.println(str1.substring(6));
		System.out.println(str1.substring(6, 9));
		
		//문자열.replace(대상문자, 변경할문자) : String
		//문자열에서 대상문자를 찾아서 변경할 문자로 반환된 문자열 반환
	...
	public void method03() {
		...
		//분리되 문자열 배열을 다시 String으로 변경하는 방법
		//String.join(구분자, 배열);
		String str2 = String.join(", ", arr);
		System.out.println(str2);
		
		// 방법2. 불리된 문자열들을 각각 토큰으로써 관리가능
