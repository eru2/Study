...
public class A_for {
	...
	/*
	 * <반복문>
	 * 프로그램 프름을 제어하는 제어문 중 하나
	 * 어떤 실행크드를 바녹적으로 수행시켜준다.
	 * 
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		 * 자바의 지역변수
		 * - 특정 메소드나 블록(중괄호{}) 내부에서 선언된다
		 * - 선언된 블로({})범위 내에서만 접근이 가능하고 해당 블록이 종료되면 함께 사라진다.
		 */
		...
		*/
		...
		/*
		int sum = 0;
		System.out.print("아메리카노 몇잔을 구매하시겠습니까?(가격 1500원) : ");
		int count = sc.nextInt();
		for(int i = 0; i < count; i++) {
			sum += 1500;
			System.out.println(i + "번째 반복중.... 누적금액 : " + sum);
		}
		System.out.println(count + " 잔의 아메리카노를 구매하셔서 " + sum + "원입니다.");
		*/
		...
		*/
		...
		System.out.println("1부터 n까지 더한 값 : " + sum);
		*/
		
		/*
		 * java.lang.Math 클래스에서 제공하는 random()함수를 호출하면 매번 다른 랜덤값을 받을 수 있다.
		 * 
		 * Math.random() 호출시 -> 0.0~0.9999999999999999사이의 랜덤값을 반환!
		 * 
		 * int num = Math.random(); //double형의 random수이기 때문에 int로 받을 수 없음
		 * 
		 * int num = (int)Math.random(); -> (0.0~0.9999999999999999) -> 0 ~ 0
		 * int num = (int)(Math.random() * 10); (0.0~0.9999999999999999) -> 1 ~ 10
		 * 최솟값~최대값 랜덤 수 : (int)(Math.random() * (최대값 + 1 - 최소값) + 최소값
		 */
		...
		*/
		...
		/*
		//랜덤한 숫자를 (5~50)한 숫자 n을 생성 후 1부터 n까지의 수중 짝수만 출력
