...
--부서코드가 'D2'인 사원들과 사원 또는 'D6'인 사원의 이름, 부서코드를 조회
...
-- 그룹함수
-- 1. ROLLUP
-- ROLLUP은 계층적인 그룹화를 수행하며, GROUP BY 로 묶인 컬럼에 따라서 단계적으로 그룹을 집계함.
...
-- GROUP BY ROLLUP(A, B, C) : 전체합계, 컬럼A에 따른 집계, 컬럼(A, B)에 따른 집계, 컬럼(A,B,C)에 대한 집계
-- GROUP BY ROLLUP(A, (B, C)) : 전체합계, 컬럼A에 따른 집계, 컬럼(A,(B,C))에 대한 집계
-- GROUP BY A, ROLLUP(B) : A그룹별, 전체합, B에 따른 집계

-- 2. CUBE
-- 조합가느한 모든 값에 대한 다차원 집계
-- GROUP BY CUBE(A) : 전체합계, 컬럼A에 대한 집계
-- GROUP BY BUCE(A, B) : 전체합계, 컬럼A에 대한 집계, 컬럼B에 대한 집계, 컬럼 (A,B)에 대한 집계
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
